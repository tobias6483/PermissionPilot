...
- Confirm selecting an app-scoped permission defaults the app list to `Granted` when granted apps exist, `Denied` when only denials are recorded, and `Recorded` otherwise, with `Any` still available to show not-recorded apps.
