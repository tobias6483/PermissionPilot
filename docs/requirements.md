...
| Evidence-source separation | Implemented | App-scoped TCC permissions, global/system settings, installed app identity, and background execution items are treated as separate evidence channels so global settings are not shown as per-app grants. |
