...
| System Settings manual QA status | Implemented | Runtime-only local status per permission for untested, tested working, and tested failed links. |
...
- User can mark System Settings link manual QA status locally during release checks.
