...
| Evidence-source separation | Implemented | App-scoped TCC permissions, global/system settings, installed app identity, and background execution items are treated as separate evidence channels so global settings are not shown as per-app grants, app-row badges, app-list filters, or selected-app permission rows. |
...
| Dashboard selection behavior | Implemented | Permission rows, scan summary apps, scan summary background items, scan summary stale items, installed apps, and background items are explicit clickable selections with visible selected state. Selecting an app-scoped permission focuses first on granted apps, then denied apps, then other recorded states instead of flooding the list with not-recorded apps. |
